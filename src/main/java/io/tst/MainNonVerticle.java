...
public class MainNonVerticle {
	public static void main(String[] args) {
		...
		client.getConnection(connRes -> {
			...
			try {
				// io.vertx.reactivex.pgclient.PgConnection cast
				PgConnection pgConn = (PgConnection) sqlConnection;
			} catch (Throwable e) {
				// fails
				e.printStackTrace();
			}
			...
			io.vertx.pgclient.PgConnection pgConnection = (io.vertx.pgclient.PgConnection) sqlConnection;
			} catch (Throwable e) {
				// fails
				e.printStackTrace();
			}
			try {
				PgConnectionImpl pgConn = (PgConnectionImpl) (Object)sqlConnection;
			} catch (Exception e) {
				e.printStackTrace();
			}
			client.close();
